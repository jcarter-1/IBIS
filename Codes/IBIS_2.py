...
from ibis_codes_KDE import IBIS_Configuration
from ibis_codes_KDE import IBIS_Bounds_and_Uncertainties
from ibis_codes_KDE import IBIS_Thoth_Exploration
from ibis_codes_KDE import IBIS_MCMC_Initial_Th
from ibis_codes_KDE import IBIS_strat
from ibis_codes_KDE import USeries_Age_Equations
import os
import pickle
import numpy as np
...
class IBIS:
    def __init__(self, filepath, sample_name, MCMC_samples = 5000, MCMC_burn_in = 1000, MCMC_Strat_samples = 50000,
        ...
        if self.Hiatus_Check: 
            self.Hiatus_start = input("Please input start (highest popint) of the observed Hiatus \
                                    Match units of the input data: ")
            self.Hiatus_end = input("Please input end (lowest popint) of the observed Hiatus \
                                    Match units of the input data: ")
    ...
    def Setup_Bounds_and_Uncertainties(self): 
        if os.path.exists(self.bounds_file_name):
            ...
            self.bounds_ibis_ext = IBIS_Bounds_and_Uncertainties.IBIS_bounds_and_Uncertainties(r08, r48,
            r08_err, r48_err, self.bounds_file_name)
            
            self.bounds_ibis_ext.save_bounds()  # Ensure this method computes and saves the data to the file
            with open(self.bounds_file_name, 'rb') as f:
    ...
    def Initialize_Thoth(self): 
        if os.path.exists(self.kdes_name):
            # If the file exists, load the bounds and uncertainties
            with open(self.kdes_name, 'rb') as input:
                self.Thor_prior = pickle.load(input)
                print('Thorium Prior Loaded')
                
        else: 
            self.thoth = IBIS_Thoth_Exploration.IBIS_Thoth_Robust(self.df_reduced,
                                num_samples =5000,
                                file_name = self.kdes_name)

            self.thoth.save_thor_prior()  # Ensure this method computes and saves the data to the file
            with open(self.kdes_name, 'rb') as f:
                self.Thor_prior = pickle.load(f)
                print('Thorium Prior computed and saved.')
    ...
    def Get_Speleo_Stats(self): 
        self.thoth = IBIS_Thoth_Exploration.IBIS_Thoth_Robust(self.df_reduced,
                                num_samples =5000,
                                file_name = self.kdes_name)
        return self.thoth.Ks_Test()
    ...
    def Run_MCMC_Strat(self): 
        ...
        if self.Hiatus_Check:
            self.Ibis_Stratigraphy = IBIS_strat_Hiatus.IBIS_strat_Hiatus(U_ages, 
                                               U_ages_low,
                                               U_ages_high,
                                               self.df_reduced, 
                                                self.sample_name,
