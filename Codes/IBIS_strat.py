...
class IBIS_Strat:
    def __init__(self, U_series_ages,
        ...
        if self.Top_Age_Stal:
            ...
            self.low_age_err = np.insert(U_series_ages_err_low, 0, 0)
            self.high_age_err = np.insert(U_series_ages_err_high, 0, 0.1)
            ...
            self.low_age_err = U_series_ages_err_low
            self.high_age_err = U_series_ages_err_high
    ...
    def Log_Priors(self, theta):
        ...
        max_ages = self.u_ages[-1] + 2*self.high_age_err[-1]
        min_ages = self.u_ages[0] - 2*self.low_age_err[0]
    
        lp += np.sum(uniform.logpdf(Age_Model,
                                    loc = min_ages,
                                    scale = max_ages - min_ages))
    
        lp += np.sum(uniform.logpdf(Depth_Model,
                                    loc = 0,
                                    scale = self.Depths.max() + 2* self.Depths_err[-1]))
    ...
    def Initial_Guesses_for_Model(self):
        ...
        for i in range(self.n_chains):
            ...
            while log_prior == -np.inf:
                ...
                Depth_Model = np.linspace(self.Depths[0], self.Depths[-1], len(self.Depths)*8)
    ...
    def MCMC(self, theta, niters, chain_id):
        ...
        acceptance_counters = {param: 0 for param in tuning_factors}
        acceptance_rate_ema = {param: 0 for param in tuning_factors.keys()}  # EMA of acceptance rates
        ...
        for i in range(1, niters):
            ...
            if i % 3000 == 0 and i != 0:  # Adaptation frequency
                for param in tuning_factors.keys():
                    acceptance_rate_ema[param] = acceptance_counters[param] / (3000 / len(tuning_factors))
                    if acceptance_rate_ema[param] > 0.4:
                        tuning_factors[param] *= 1.1
                    else:
                        tuning_factors[param] *= 0.9
                        
                    tuning_factors[param] = max(tuning_factors[param], 1e-8)
                    tuning_factors[param] = min(tuning_factors[param], 1000)
                    
            acceptance_counters[specific_counter_name] += 1
            ...
            if (i + 1) % 5000 == 0:
                with open(f'{self.sample_name}_Age_Depth_theta_{chain_id}.pkl', 'wb') as f:
                    pickle.dump(theta, f)
                with open(f'tuning_factor_Age_Depth_{chain_id}.pkl', 'wb') as f:
    ...
    def Get_Age_Model(self):
        ...
        c_age_m = np.concatenate(Age_m)
        
        low_age, high_age = np.percentile(c_age_m, [2.5, 97.5], axis = 0)
