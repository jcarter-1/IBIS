...
class U_Series_Age_Equation:
    def __init__(self, r08, r08_err,
        ...
        self.lambda_230 = 9.17055e-06   # Cheng et al. (2013)
        self.lambda_234 = 2.82203e-06 # Cheng et al. (2013)
        self.lambda_230_err = 6.67e-09 # Cheng et al. (2013)
        self.lambda_234_err = 1.494e-09 # Cheng et al. (2013)
...
class IBIS_Thoth_Robust:
    ...
    THOTH
    ======
    This function forms the prior for the speleothem
    if method = 'thoth' is preset (always recommend it is most flexible)
    
    Read in the bounds that were made - maximum age of the speleothem (lowest age in depth space + 5𝜎)
    
    Global
    ------
    Builds a sequence of (230Th/232Th)_{A0} that satisfy superposition for the whole speleothem.(230Th/232Th)_{A0} samples are drawn from a boutique sampler.
    
    Segement
    --------
    Segment - if the datapoints are arranged from top to bottom in a sequence of i = 0, 1, ..., N-1, N, then a sequence is defined as i = 0, 1, 2 and i = 1, 2, 3 up to i = N-2, N-1, N.
    Builds sequences of (230Th/232Th)_{A0} for each segment that satisfy stratigraphic constraints.
    
    This is built for the possibility that the initial thorium is variable throughout. Secondly, it is good back up if the global builder fails - nice fallback to still get a reasonable represenation of the empirical Bayes (230Th/232Th)_{A0} prior.
    
    Thoth
    -----
    Combines the global and segement to get the best "guess" prior for the sample.
    ...
    def __init__(self, data, age_max, num_samples = 1000,
        ...
        self.data = data
        self.Th230_lam_Cheng = 9.17055e-06   # Cheng et al. (2013)
        self.Th230_lam_Cheng_err = 6.67e-09 # Cheng et al. (2013)
        self.U234_lam_Cheng = 2.82203e-06 # Cheng et al. (2013)
        self.U234_lam_Cheng_err = 1.494e-09 # Cheng et al. (2013)
    ...
    def _strat_loglik(self,  ages, errs):
        ...
        for i in range(N-1):
            for j in range(i+1, N):
                σ    = np.sqrt(errs[i]**2 + errs[j]**2) # Uncertainties between age steps added in quadrature
                Δ    = ages[j] - ages[i]
    ...
    def uniform_log_sample(self, lower=0.1, upper=50, n_samp = 1):
        """
    ...
    def save_thor_prior(self):
        ...
        print(f"Thorium prior saved to {fname}")
    ...
    def load_thor_prior(self):
        ...
        if os.path.exists(fname):
            ...
            print(f"Thorium prior loaded from {fname}")
        else:
            print(f" No prior at {fname}; computing & saving now.")
